...
class Tester:
    def __init__(self, args):
        ...
        self.device = 'cpu' #torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu') #i set this to cpu previously
    ...
    def run_detector(self, all_image_folder):
        ...
        bboxes = []
        for fold_id, image_folder in enumerate(glob(os.path.join(all_image_folder[0], "*"))):
    ...
    def run_on_image_folder(self, all_image_folder, detections, output_folder, visualize_proj=True):
        all_image_folder = glob(os.path.join(all_image_folder[0], "*"))
        print(f"all_image_folder: {all_image_folder}")
        ...
        for fold_idx, image_folder in enumerate(all_image_folder):
            ...
            print(f"image_file_names len: {len(image_file_names)}")
            ...
            for img_idx, img_fname in tqdm.tqdm(enumerate(image_file_names)):
                ...
                img_h = torch.tensor(orig_height).repeat(batch_size).float()
                img_w = torch.tensor(orig_width).repeat(batch_size).float()
                ...
                #print(f"hmr_output: {hmr_output['vertices'].shape}")
                ...
                output_folder = "/home/cankeles/bedlam_smpl/BEDLAM/smpl_output"
                #smpl_mesh = trimesh.Trimesh(vertices=pred_vertices_array[0], faces=self.smplx_cam_head.smplx.faces)
                #smpl_mesh_path = os.path.join(output_folder, f"smpl_model_{img_idx}.obj")
                #smpl_mesh.export(smpl_mesh_path)

                for idx in range(pred_vertices_array.shape[0]):
                    renderer = Renderer(focal_length=focal_length[0], img_w=img_w[0], img_h=img_h[0],
                                        faces=self.smplx_cam_head.smplx.faces,
                                        same_mesh_color=False)

                    print(f"input shape for rendere: {np.expand_dims(pred_vertices_array[idx], axis=0).shape}, pred_vertices_array.shape: {pred_vertices_array.shape}")
                    front_view, depth_map = renderer.render_front_view(np.expand_dims(pred_vertices_array[idx], axis=0),
                                                            bg_img_rgb=img.copy())

                    #Added by Can Keles
                    print(f"depth_map.shape: {depth_map.shape}, depth min: {depth_map.min()}, depth max: {depth_map.max()}")
                    depth_map *= 256
                    depth_map = (depth_map).astype(np.uint16)

                    print(f"img_fname: {os.path.normpath(img_fname).split(os.sep)[-2]}, img_fname full: {img_fname}")
                    
                    output_folder_n = os.path.join(output_folder, os.path.normpath(img_fname).split(os.sep)[-2])
                    if not os.path.exists(output_folder_n):
                        os.mkdir(output_folder_n)

                    cv2.imwrite(os.path.join(output_folder_n, f"{img_fname.split('/')[-1]}_depth_map_{idx}.png"), depth_map)
                    
                    basename = img_fname.split('/')[-1]
                    filename_orig = basename + "orig_%s.jpg" % 'bedlam'
                    front_view_path = os.path.join(output_folder_n, f"{img_fname.split('/')[-1]}_pred_{idx}.png") 
                    orig_path = os.path.join(output_folder_n, f"{img_fname.split('/')[-1]}_orig_{idx}.png") 
                ...
                renderer = Renderer(focal_length=focal_length[0], img_w=img_w[0], img_h=img_h[0],
                                    faces=self.smplx_cam_head.smplx.faces,
                                    same_mesh_color=False)
                front_view, depth_map = renderer.render_front_view(pred_vertices_array,
                                                        bg_img_rgb=img.copy())
                ...
                depth_map *= 256
                depth_map = (depth_map).astype(np.uint16)
                cv2.imwrite(os.path.join(output_folder, f"{img_fname.split('/')[-1]}_depth_map.png"), depth_map)
                """
                """
    ...
    def run_on_dataframe(self, dataframe_path, output_folder, visualize_proj=True):
        ...
        for ind, center in tqdm.tqdm(enumerate(centers)):
            ...
            img_h = torch.tensor(orig_height).repeat(1).float()
            img_w = torch.tensor(orig_width).repeat(1).float()
